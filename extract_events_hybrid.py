...
def main():
    ...
    # HybridRetriever uses embeddings + keywords
    retriever = HybridRetriever(chunk_size=2000, overlap=200, alpha=0.7)
    extractor = EventExtractor() # Re-use the same extractor (Gemini)
    ...
    for doc in documents:
        ...
        # Handle 'from' vs 'from_'
        author = doc.get('from', doc.get('from_', 'Unknown'))
        ...
        for event in EVENTS:
            # 1. Retrieve relevant chunks using Hybrid Strategy
            # This will filter by keywords first, then rank top 10 candidates by embedding similarity
            ...
            print(f"  + Found relevant text for {event} (Hybrid). Extracting...")
    ...
    # Save results
    output_path = os.path.join(OUTPUT_DIR, "events_hybrid.json")
    with open(output_path, 'w', encoding='utf-8') as f:
        json.dump(all_extractions, f, indent=2, ensure_ascii=False)
        
    print(f"\nHybrid Extraction complete. Saved {len(all_extractions)} events to {output_path}")
